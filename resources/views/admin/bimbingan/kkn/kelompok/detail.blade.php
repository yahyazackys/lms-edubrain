...
@section('content')
    <div class="min-h-screen bg-gray-50 py-6">
                                            ...
                                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="flex items-center">
                                                        <div class="flex-shrink-0 h-10 w-10">
                                                            @if ($detail->pesertaBimbingan->registrasiMahasiswa->mahasiswa->foto)
                                                                <img class="h-10 w-10 rounded-full object-cover"
                                                                    src="{{ asset('storage/foto-mahasiswa/' . $detail->pesertaBimbingan->registrasiMahasiswa->mahasiswa->foto) }}"
                                                                    alt="">
                                                            @else
                                                                <div
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <div class="flex items-center justify-center space-x-2">
                                                        <button
                                                            onclick="openEditPeranModal('{{ $detail->id_kkn_detail }}', '{{ $detail->peran_kelompok }}', '{{ $detail->pesertaBimbingan->registrasiMahasiswa->mahasiswa->pengguna->nama }}')"
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                        @foreach ($dokumentasi['images'] as $image)
                                            <div class="relative group cursor-pointer border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow"
                                                onclick="openImageModal('{{ asset('storage/' . $image->file_path) }}', '{{ addslashes($image->judul) }}', '{{ addslashes($image->deskripsi ?? '') }}')">
        ...
        <div id="tambahPesertaModal" class="fixed inset-0 z-[9999] overflow-y-auto hidden">
            <div class="flex items-center justify-center min-h-screen p-4">
                ...
                <div
                    ...
                    <form method="POST" action="{{ route('bimbingan.kkn.peserta.assign') }}">
                        @csrf
                        <input type="hidden" name="id_kelompok_kkn" value="{{ $kelompok->id_kelompok_kkn }}">
        ...
        <!-- Modal Edit Peran -->
        <div id="editPeranModal" class="fixed inset-0 z-[9999] overflow-y-auto hidden">
            <div class="flex items-center justify-center min-h-screen p-4">
                <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onclick="closeEditPeranModal()">
                </div>

                <div
                    class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all w-full max-w-md">
                    <form method="POST" action="{{ route('bimbingan.kkn.peserta.update-peran') }}">
                        @csrf
                        <input type="hidden" name="id_kkn_detail" id="edit_id_kkn_detail">

                        <div class="bg-white px-6 pt-6 pb-4">
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="text-lg font-semibold text-gray-900">Edit Peran Mahasiswa</h3>
                                <button type="button" onclick="closeEditPeranModal()"
                                    class="text-gray-400 hover:text-gray-600">
                                    <i class="fas fa-times"></i>
                                </button>
                            </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-2">Mahasiswa</label>
                                    <div class="text-sm text-gray-900 font-medium" id="edit_nama_mahasiswa"></div>
                                </div>

                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-2">
                                        Peran Baru <span class="text-red-500">*</span>
                                    </label>
                                    <select name="peran_kelompok" id="edit_peran_kelompok" required
                                        class="w-full text-xs px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900">
                                        <option value="">Pilih Peran</option>
                                        <option value="KETUA">Ketua</option>
                                        <option value="ANGGOTA">Anggota</option>
        ...
        <form id="delete-peserta-form" method="POST" style="display: none;">
            @csrf
            <input type="hidden" name="id_kkn_detail" id="delete_id_kkn_detail">
        </form>
    </div>
    ...
    @push('scripts')
        <script>
            ...
            // Edit Peran Modal
            function openEditPeranModal(idDetail, currentPeran, namaMahasiswa) {
                document.getElementById('edit_id_kkn_detail').value = idDetail;
                document.getElementById('edit_nama_mahasiswa').textContent = namaMahasiswa;
                document.getElementById('edit_peran_kelompok').value = currentPeran;
                document.getElementById('editPeranModal').classList.remove('hidden');
            }

            function closeEditPeranModal() {
                document.getElementById('editPeranModal').classList.add('hidden');
            }
            ...
            function loadMahasiswaData() {
                fetch('/bimbingan/kkn/peserta/api/mahasiswa-tersedia?semester={{ $kelompok->id_semester }}')
                ...
                if (data.length === 0) {
                    ...
                    dropdown.innerHTML = data.map(item => `
                        <div class="px-4 py-3 hover:bg-gray-50 cursor-pointer border-b border-gray-100 last:border-b-0" 
                             onclick="selectMahasiswa('${item.id_peserta_bimbingan}', '${escapeHtml(item.nama)}', '${item.nim}')">
            ...
            function confirmRemovePeserta(idDetail, namaMahasiswa) {
                if (confirm(`Yakin ingin mengeluarkan ${namaMahasiswa} dari kelompok?`)) {
                    ...
                    form.action = '{{ route('bimbingan.kkn.peserta.remove') }}';
                    document.getElementById('delete_id_kkn_detail').value = idDetail;
                    form.submit();
                }
            }
            ...
            window.openEditPeranModal = openEditPeranModal;
            window.closeEditPeranModal = closeEditPeranModal;
