...
@section('title', 'Profil Saya - ' . $mahasiswa->pengguna->nama)
...
@section('content')
    <div class="min-h-screen bg-gray-50 py-6">
        <div class="mx-auto">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-100 font-heading">
                ...
                <!-- Tab Content -->
                <form id="profileForm" method="POST" action="{{ route('mahasiswa.biodata.update') }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div id="content-data-utama" class="tab-content active">
                        <div class="p-4 sm:p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                <!-- NIM -->
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-2">NIM</label>
                                    <div
                                ...
                                <!-- Kurikulum -->
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-2">Kurikulum</label>
                                    <div
                                        class="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-900">
                                ...
                                <!-- Angkatan -->
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-2">Angkatan</label>
                                    <div
                                        class="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-900">
                                        {{ $mahasiswa->angkatan }}
                                    </div>
                                ...
                                <!-- Status -->
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-2">Status Mahasiswa</label>
                                    <div class="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-xs">
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ match ($mahasiswa->status_mahasiswa) {
                                                'AKTIF' => 'bg-green-100 text-green-800',
                                                'CUTI' => 'bg-yellow-100 text-yellow-800',
                                                'DO' => 'bg-red-100 text-red-800',
                                                'KELUAR' => 'bg-gray-100 text-gray-800',
    ...
    <div id="password-modal"
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                ...
                <form id="password-form" method="POST" action="{{ route('mahasiswa.biodata.update-password') }}">
                    @csrf
                    @method('PUT')
    ...
    @push('scripts')
        <script>
            ...
            function isValidTab(tabName) {
                const validTabs = ['data-utama', 'data-pribadi', 'data-orangtua', 'data-wali', 'kontak'];
    ...
    `;
                ...
                fetch(form.action, {
                    ...
                    .then(data => {
                        if (data.success) {
                            // ✅ Gunakan alert sistem yang sudah ada
                ...
                fetch(form.action, {
                    ...
                    .then(data => {
                        if (data.success) {
                            // ✅ Gunakan alert sistem yang sudah ada
                            showSuccess(data.message);
                            // Reset and close password modal
                            form.reset();
                            closePasswordModal();
                        } else {
                            // ✅ Gunakan alert sistem yang sudah ada
            ...
            function uploadPhoto() {
                ...
                fetch('{{ route('mahasiswa.biodata.upload-photo') }}', {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                        },
            ...
            function deletePhoto() {
                if (confirm('Yakin ingin menghapus foto profil?')) {
                    fetch('{{ route('mahasiswa.biodata.delete-photo') }}', {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
